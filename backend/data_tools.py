...
class PriceHandler:
    ...
    def get_prices(self, initial_investments: Dict[str, float]) -> "PriceHandler":
        ...
        raw_prices: Dict[str, Dict[str, float]] = self.cg.get_price(
            ids=self.coin_ids, vs_currencies=self.vs_currency
        )
        self.prices = {
            key: value[self.vs_currency] for key, value in raw_prices.items()
        }
        logger.info(f"Got prices {self.prices}")
        return self

    def calculate_returns(
        self, initial_investments: Dict[str, float]
    ) -> Dict[str, float]:
...
class DataReader:
    ...
    @staticmethod
    def get_initial_investments_from_source(
        filename: str, source: str = "local"
    ) -> Dict[str, float]:
        if source == "local":
            ...
            if os.path.exists(initial_inv_list):
                ...
                return initial_dict
            logger.warning(
                "Initial_investments file was not found, using defaults instead."
            )
            ...
            body = (
                s3_resource.Object(s3_settings.S3_BUCKET, filename)
                .get()["Body"]
                .read()
                .decode("utf-8")
            )
